        ...
        public void send(MidiMessage msg, long timeStamp) {
            if (msg instanceof ShortMessage sm) {
                if (((ShortMessage) msg).getCommand() == CONTROL && midiControlHandler != null) {
                    ...
                    switch (data) {
                        case DATA_BEGINNING ->
                                Minecraft.getInstance().submitAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.BEGINNING));
                        case DATA_END ->
                                Minecraft.getInstance().submitAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.END));
                        case DATA_STOP ->
                                Minecraft.getInstance().submitAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.STOP));
                        case DATA_PREVIEW ->
                                Minecraft.getInstance().submitAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.PREVIEW));
                        case DATA_RECORD ->
                                Minecraft.getInstance().submitAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.RECORD));
                    }
                    return;
                }
