...
public class MidiHandler
{
    ...
    public class MidiInputReceiver implements Receiver {
        ...
        private float volumeCurve(float x) {
        ...
        public void send(MidiMessage msg, long timeStamp) {
            if (msg instanceof ShortMessage) {
                ...
                if(((ShortMessage) msg).getCommand() == CONTROL && midiControlHandler != null){
                    ...
                    switch (data){
                        case DATA_BEGINNING: {
                        	Minecraft.getInstance().runAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.BEGINNING));
                        	break;
                        }
                        case DATA_END: {
                        	Minecraft.getInstance().runAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.END));
                        	break;
                        }
                        case DATA_STOP: {
                        	Minecraft.getInstance().runAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.STOP));
                        	break;
                        }
                        case DATA_PREVIEW: {
                        	Minecraft.getInstance().runAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.PREVIEW));
                        	break;
                        }
                        case DATA_RECORD: {
                        	Minecraft.getInstance().runAsync(() -> midiControlHandler.accept(GuiMusicSheet.MidiControl.RECORD));
                        	break;
                        }
                    }
                    return;
                ...
                if (sm.getCommand() == NOTE_ON && velocity > 0) {
                    ...
                    Minecraft.getInstance().runAsync(() -> noteOnHandler.accept(new MidiData(key, vol)));
                } else if (sm.getCommand() == NOTE_OFF || (sm.getCommand() == NOTE_ON && velocity == 0)) {
                    Minecraft.getInstance().runAsync(() -> noteOffHandler.accept(key));
                }
            }
        }
    ...
    public static class MidiData {
    	public int noteId;
    	public float volume;
    	public MidiData(int noteId, float volume) {
    		this.noteId = noteId;
    		this.volume = volume;
    	}
    }
