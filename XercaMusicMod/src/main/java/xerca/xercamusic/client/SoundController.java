...
public class SoundController extends Thread {
    ...
    public void run() {
        ...
        if (spiritID >= 0 && minecraft.player != null) {
            minecraft.submit(() -> ClientStuff.endMusic(spiritID, minecraft.player.getId()))
                    .whenComplete((v, t) -> {
                        if (t != null) XercaMusic.LOGGER.error("Failed to end music", t);
                    }).isDone();
        }
    }
    ...
    private void playNotes(int fromIdx, int toIdx) {
        Minecraft.getInstance().submit(() -> {
            ...
            if (t != null) XercaMusic.LOGGER.error("Failed to play notes", t);
        }).isDone();
    }
    ...
    private void updateActiveSounds(int currentBeat) {
        ...
        Minecraft.getInstance().submit(() -> {
            ...
            if (t != null) XercaMusic.LOGGER.error("Failed to update active sounds", t);
        }).isDone();
    }
    ...
    private void sleepUntil(long targetNanos) {
        ...
        if (remainingMs > 8) {
            try {
                ...
                XercaMusic.LOGGER.warn("Interrupted while sleeping", e);
