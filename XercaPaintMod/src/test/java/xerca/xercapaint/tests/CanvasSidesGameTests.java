...
public class CanvasSidesGameTests {
    ...
    private static final RecipeCanvasCloning CLONING_RECIPE = new RecipeCanvasCloning(new ResourceLocation(MODID, "test_canvas_cloning"), CraftingBookCategory.MISC);
    ...
    public static void canvasUpdatePacketRoundTripsSides(GameTestHelper helper) {
        ...
        CanvasUpdatePacket original = new CanvasUpdatePacket(pixels, false, "", "canvas-sides", 3, null, palette, type, true, sidePixels);
        FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.buffer());
        CanvasUpdatePacket.encode(original, buf);
        CanvasUpdatePacket decoded = CanvasUpdatePacket.decode(buf);

        helper.assertTrue(decoded != null && decoded.isMessageValid(), "Packet must decode");
        helper.assertTrue(decoded.isSidesActive(), "sidesActive must survive packet round-trip");
        helper.assertTrue(Arrays.equals(decoded.getSidePixels(), sidePixels), "Side pixels must survive packet round-trip");
        helper.assertTrue(Arrays.equals(decoded.getPixels(), pixels), "Front pixels must survive packet round-trip");
        helper.succeed();
    }
    ...
    public static void cloningCopiesSidePixels(GameTestHelper helper) {
        ...
        originalTag.putString(ItemCanvas.TAG_CANVAS_ID, "sides_clone");
        ...
        originalTag.putBoolean(ItemCanvas.TAG_SIDES_ACTIVE, true);
        int[] sideList = sampleSidePixels(type);
        originalTag.putIntArray(ItemCanvas.TAG_SIDE_PIXELS, sideList);

        ItemStack result = CLONING_RECIPE.assemble(createGrid(2, 2, original, new ItemStack(Items.ITEM_CANVAS.get())),
                helper.getLevel().registryAccess());

        helper.assertTrue(!result.isEmpty(), "Expected a clone result");
        helper.assertTrue(result.getOrCreateTag().getBoolean(ItemCanvas.TAG_SIDES_ACTIVE), "Clone must copy sidesActive");
        helper.assertTrue(java.util.Arrays.equals(sideList, result.getOrCreateTag().getIntArray(ItemCanvas.TAG_SIDE_PIXELS)), "Clone must copy side pixels");
        helper.succeed();
    }
    ...
    public static void glassFlagSurvivesEntityNbtRoundTrip(GameTestHelper helper) {
        ItemStack glassStack = new ItemStack(Items.ITEM_CANVAS_GLASS.get());
        ...
        CompoundTag glassTag = glassStack.getOrCreateTag();
        glassTag.putString(ItemCanvas.TAG_CANVAS_ID, "glass_entity");
        glassTag.putInt(ItemCanvas.TAG_VERSION, 1);
        glassTag.putIntArray(ItemCanvas.TAG_PIXELS, new int[area]);

        BlockPos pos = helper.absolutePos(new BlockPos(1, 2, 1));
        EntityCanvas canvas = new EntityCanvas(helper.getLevel(), glassTag, pos, Direction.NORTH, CanvasType.SMALL, itemCanvas.isGlass(), 0);
        ...
        EntityCanvas reloaded = new EntityCanvas(Entities.CANVAS.get(), helper.getLevel());
